    ...
    static class UpdateHandler extends Handler
    {
        ...
        public void handleMessage(Message msg)
        {
            ...
            if (null != layout)
            {
                ...
                {
                    // 真正开始进行刷新和加载
                    if (layout.state == RELEASE_TO_REFRESH
                            && layout.pullDownY <= layout.refreshDist)
                    {
                        layout.changeState(REFRESHING);
                        // 刷新操作
                        if (layout.mListener != null)
                            layout.mListener.onRefresh(layout);
                    }
                    if (layout.state == RELEASE_TO_LOAD
                            && -layout.pullUpY <= layout.loadmoreDist)
                    {
                        layout.changeState(LOADING);
                        // 加载操作
                        if (layout.mListener != null)
                            layout.mListener.onLoadMore(layout);
                    }
